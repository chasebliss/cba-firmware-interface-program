// Chase Bliss Audio — Bliss Programmer UI Kit Components
// Shared React components — exported to window for use in index.html

const CBA_LOGO = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2315 307" aria-label="Chase Bliss Audio" style={{width:'208px',height:'32px',display:'block'}}>
...
const FIRMWARES = [
  { name: 'MOOD MKII',   bgColor: '#b48ecf' },
  { name: 'Lossy',       bgColor: '#e87fa0' },
  { name: 'Blooper',     bgColor: '#7ab8d4' },
  { name: 'Thermae',     bgColor: '#c4a35a' },
  { name: 'Automatone',  bgColor: '#5db88e' },
  { name: 'Preamp MKII', bgColor: '#d4845a' },
];
...
function Nav({ beta }) {
  return (
    <nav style={{
      ...
      <div style={{width:'33%'}}>
        <a href="#" style={{display:'flex',alignItems:'center',gap:4,textDecoration:'none',color:'#000',fontWeight:700,fontSize:14}}>
      ...
      <div style={{width:'33%',display:'flex',justifyContent:'center'}}>
        <a href="https://chasebliss.com" target="_blank"><CBA_LOGO /></a>
      </div>
      <div style={{width:'33%',display:'flex',justifyContent:'flex-end',alignItems:'center',gap:8,fontWeight:700,fontSize:14}}>
        <span>Instructions:</span>
        <svg fill="#000" viewBox="-3.5 -2 24 24" width="28" height="28">
        ...
        <svg fill="#000" viewBox="0 0 14 14" width="26" height="26">
...
function BetaBanner() {
  return (
    <div style={{
      margin: '0 -7vw', padding: '10px 7vw',
      background: '#000', color: '#fefbf6',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      gap: 12, fontSize: 14, fontWeight: 500,
    }}>
      <span style={{background:'#fefbf6',color:'#000',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.12em',padding:'2px 8px',fontSize:11}}>Beta</span>
      <span>Unreleased firmware. For internal testing only, not for customer devices.</span>
    </div>
  );
}

// ── DropdownSelect ───────────────────────────────────────────
function DropdownSelect({ selected, onSelect }) {
  ...
  return (
    <div style={{position:'relative',width:384}}>
      <div
        ...
        onMouseEnter={() => {}}
        style={{
          border: '2px solid #000', background: '#fefbf6',
          padding: '12px 14px',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          fontWeight: 700, cursor: 'pointer',
          boxShadow: (open || selected) ? '0 20px 25px -5px rgb(0 0 0/.1),0 8px 10px -6px rgb(0 0 0/.1)' : 'none',
          transition: 'box-shadow 0.3s ease-in-out',
        }}>
        {selected ? selected.name : 'Select Pedal...'}
        <svg viewBox="0 0 20 20" width="24" height="24"
          style={{transform: open ? 'rotate(180deg)' : 'none', transition:'transform 0.15s ease'}}>
          <path d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-4.293-1.707a1 1 0 00-1.414 0L10 10.586 7.707 8.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 000-1.414z" fill="#ba8e51"/>
        </svg>
      </div>
      {open && (
        <div style={{
          position:'absolute', top:'100%', left:0, right:0, zIndex:10,
          border:'2px solid #000', borderTop:'none', background:'#fefbf6',
        }}>
          {FIRMWARES.map((fw, i) => (
            <div key={fw.name}
              style={{
                padding: '12px 16px',
                borderBottom: i < FIRMWARES.length-1 ? '1px solid #000' : 'none',
                fontWeight: 700, cursor: 'pointer',
                background: hovered === fw.name ? fw.bgColor : '#fefbf6',
                transition: 'background 0.15s',
              }}
              onMouseEnter={() => setHovered(fw.name)}
              onMouseLeave={() => setHovered(null)}
              onClick={() => { onSelect(fw); setOpen(false); }}>
              {fw.name}
            </div>
          ))}
        </div>
...
// ── CBAButton ─────────────────────────────────────────────────
function CBAButton({ children, disabled, variant, onClick }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        background: '#fefbf6',
        border: `2px solid ${isSuccess ? '#10b981' : '#000'}`,
        color: isSuccess ? '#10b981' : '#000',
        width: 200, minWidth: 204, height: 50,
        padding: '10px',
        cursor: disabled ? 'not-allowed' : 'pointer',
        opacity: disabled ? 0.4 : 1,
        fontStyle: hov && !disabled ? 'italic' : 'normal',
        boxShadow: (hov && !disabled) || isSuccess ? '0 20px 25px -5px rgb(0 0 0/.1),0 8px 10px -6px rgb(0 0 0/.1)' : 'none',
        transition: 'box-shadow 0.3s ease-in-out',
      }}>
      {children}
    </button>
...
function BinaryHero() {
  return (
    <img src="../../assets/binaryV2.svg" alt="" style={{width:500,maxWidth:'100%',opacity:0.85}} />
  );
}

// ── ProgressOverlay ──────────────────────────────────────────
function ProgressOverlay({ firmware, progress, status }) {
  return (
    <div style={{
      display:'flex', flexDirection:'column', alignItems:'center',
      gap:12, width:384,
    }}>
      <progress value={progress} max={100} style={{width:'100%',height:15,appearance:'none'}} />
      <style>{`
        progress::-webkit-progress-bar { background: #f0ebe3; }
        progress::-webkit-progress-value { background: ${firmware ? firmware.bgColor : '#10b981'}; transition: width 0.3s; }
      `}</style>
      <p style={{
        border: '2px solid #10b981', color: '#10b981',
        fontWeight: 700, fontSize: 16, padding: '10px 16px',
        background: '#fefbf6', minWidth: 204, textAlign: 'center',
        animation: status === 'loading' ? 'cba-pulse 2s cubic-bezier(.4,0,.6,1) infinite' : 'none',
      }}>
        {status === 'loading' ? 'Uploading...' : status === 'done' ? 'Update complete.' : 'Preparing...'}
      </p>
      <style>{`@keyframes cba-pulse{0%,100%{opacity:1}50%{opacity:.5}}`}</style>
    </div>
  );
}
...
function HeadingBox({ children }) {
  return (
    <div style={{
      border:'2px solid #000', padding:'16px 20px',
      fontWeight:700, fontSize:'clamp(1rem,1.2vw + 1rem,2rem)',
      background:'#fefbf6', width:'fit-content',
...
function WarningBadge({ children }) {
  return (
    <p style={{fontStyle:'italic',fontWeight:700,fontSize:13}}>
      <span style={{background:'#ef4444',color:'#fff',fontWeight:700,padding:'1px 6px',marginRight:6,fontStyle:'normal'}}>Note:</span>
      {children}
    </p>
  );
}
...
Object.assign(window, {
  CBA_LOGO, Nav, BetaBanner, DropdownSelect, CBAButton,
  BinaryHero, ProgressOverlay, HeadingBox, WarningBadge,
  FIRMWARES,
});
