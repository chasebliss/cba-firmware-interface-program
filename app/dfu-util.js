...
(function () {
  ...
  function logInfo(
    ...
    // Clear the log container every time to ensure only the latest message is displayed
    logContext.innerHTML = "";
    ...
    if (showProgressBar && progress !== null && total !== null) {
    ...
    logContext.appendChild(infoContainer);
  }
  ...
  document.addEventListener("DOMContentLoaded", (event) => {
    ...
    async function connect(device) {
      ...
      console.log(device); // Log to console as well
      console.log(`Connecting to Device:`);
      console.log(`Vendor ID (VID): ${device.device_.vendorId}`);
      console.log(`Product ID (PID): ${device.device_.productId}`);
      console.log(`Serial Number: ${device.device_.serialNumber}`);
      console.log(`Device Product Name: ${device.device_.productName}`);
      console.log(`Device Manufacturer: ${device.device_.manufacturerName}`);

      // Log interface information
      console.log(
        `Configuration Number: ${device.settings.configuration.configurationValue}`,
      );
      console.log(
        `Interface Number: ${device.settings["interface"].interfaceNumber}`,
      );
      console.log(
        `Alternate Setting: ${device.settings.alternate.alternateSetting}`,
      );
      console.log(
        `Interface Class: ${device.settings["interface"].interfaceClass}`,
      );
      console.log(
        `Interface Subclass: ${device.settings["interface"].interfaceSubclass}`,
      );
      console.log(
        `Interface Protocol: ${device.settings["interface"].interfaceProtocol}`,
      );
      let memorySummary = "";
    ...
    //     const models = event.target;
    //     let address = parseInt(models.value, 16);
    //     if (isNaN(address)) {
    //         models.setCustomValidity("Invalid hexadecimal start address");
    //     } else if (device && device.memoryInfo) {
    //         if (device.getSegment(address) !== null) {
    //             device.startAddress = address;
    //             models.setCustomValidity("");
    //             dfuseUploadSizeField.max = device.getMaxReadSize(address);
    //         } else {
    //             models.setCustomValidity("Address outside of memory map");
    //         }
    //     } else {
    //         models.setCustomValidity("");
    //     }
